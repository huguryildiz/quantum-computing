...
const PLOT = (() => {
  ...
  /* `noise` and `dec` are what this course needs and a deterministic course does
     not. Noise takes no hue of its own: it is the hairline tone, drawn at low
     opacity behind whatever it disturbs, so the symbol points inside a cloud
     keep their own colour. A decision region is the colour of the symbol it
     decides for, at an opacity low enough to sit under a constellation point
  ...
  const LIGHT = {
    ...
    noise:'#B8B2A6', noiseSoft:'rgba(184,178,166,0.28)',
    dec:{ in:'rgba(20,112,127,0.10)', h:'rgba(192,132,34,0.11)',
          out:'rgba(74,122,70,0.10)', mid:'rgba(106,90,146,0.11)',
          err:'rgba(166,59,42,0.10)' }
  };
  const DARK = {
    ...
    noise:'#44515F', noiseSoft:'rgba(68,81,95,0.34)',
    dec:{ in:'rgba(79,190,206,0.12)', h:'rgba(229,178,85,0.12)',
          out:'rgba(130,194,123,0.12)', mid:'rgba(172,153,220,0.13)',
          err:'rgba(232,120,95,0.12)' }
  };
