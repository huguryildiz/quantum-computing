...
function figFault(){
  return P.blocks({w:760,h:250,items:[
    {t:'box',x:24,y:34,w:162,h:54,label:'physical qubits',fs:13,color:C.in},
    {t:'arrow',x1:186,y1:61,x2:222,y2:61},
    {t:'box',x:222,y:34,w:146,h:54,label:'syndrome checks',fs:13,color:C.h},
    {t:'arrow',x1:368,y1:61,x2:404,y2:61},
    {t:'box',x:404,y:34,w:132,h:54,label:'decoder',fs:13,color:C.mid},
    {t:'arrow',x1:536,y1:61,x2:572,y2:61},
    {t:'box',x:572,y:34,w:164,h:54,label:'correction',fs:13,color:C.out},
    {t:'text',x:380,y:119,label:'together these protect one logical qubit',fs:13,color:C.out},
    {t:'line',d:'M82,139 H678',color:C.rule},
    {t:'text',x:190,y:174,label:'space overhead',fs:12.5,color:C.err},
    {t:'text',x:190,y:199,label:'data qubits + check ancillas',fs:12},
    {t:'text',x:570,y:174,label:'time overhead',fs:12.5,color:C.err},
    {t:'text',x:570,y:199,label:'checks + decoding + logical gates',fs:12},
    {t:'text',x:380,y:235,label:'p < p_{\\mathrm{th}}:\\; \\text{larger code distance can suppress logical error}',tex:true,fs:13,color:C.h}
  ]});
}
...
{ id:'m5-fault', module:'M5', nav:'Physical and logical qubits', title:'One logical qubit is a protected encoding, not one better physical qubit',
  ...
  {t:'cols', ratio:'c-6-6', vcenter:true, left:[
    {t:'body', html:'<p>A <b>physical qubit</b> is a device on the chip. A <b>logical qubit</b> is quantum information encoded across a block of physical data qubits. Extra ancilla qubits measure checks on that block. The encoding is useful because a local physical error need not destroy the logical state.</p>'},
    {t:'body', html:'<p>The check results form a <b>syndrome</b>. A syndrome reveals information about an error pattern without reading the unknown amplitudes $\\alpha$ and $\\beta$. A classical decoder uses the syndrome to choose a correction. The checks, decoding and corrections repeat while the computation runs.</p>'},
    {t:'reveal', at:1, items:[
      {t:'small', html:'A code corrects only the error patterns it was designed to handle. Increasing its <b>distance</b> can protect against more faults, but it also requires more physical qubits and more check rounds. Error correction trades resources for a lower logical error rate. It does not make errors disappear for free.'}
    ]}
  ], right:[
    {t:'fig', frame:true, svg:()=>figFault(),
      caption:'The protection loop and its two costs. The logical qubit is the encoded information carried through the loop, not one of the physical devices in the first box.'},
    {t:'reveal', at:2, items:[
      {t:'note', kind:'def', head:'What the threshold means', html:'Let $p$ be the physical error rate and $p_{\\mathrm{th}}$ the threshold for a stated code, noise model and control system. If $p<p_{\\mathrm{th}}$, increasing the code distance can reduce the logical error rate. If $p$ is above that threshold, making the same code larger does not provide scalable reliability. There is no single threshold number for every machine.'}
    ]},
    {t:'reveal', at:3, items:[
      {t:'note', kind:'err', head:'Logical resources are not hardware resources', html:'A claim of logical qubits and logical gates is only the first line of a hardware estimate. It must name the code and distance, physical error assumptions, ancillas and routing. It must also name the correction-cycle time and target failure probability. Error correction expands both space and time, sometimes by many orders of magnitude.'}
    ]}
  ]}
]},
...
{ id:'m5-tele', module:'M5', nav:'The teleportation circuit', title:'Teleportation, as a circuit with three qubits and two classical wires',
  ...
  {t:'cols', ratio:'c-6-6', vcenter:true, left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'body', html:'<p><b>One.</b> A Hadamard and a CNOT make the shared pair $|\\Phi^{+}\\rangle$ on $q_{1}$ and $q_{2}$. A network distributes and verifies this pair first. Teleportation is the network primitive that later consumes the pair and two classical bits to move a state between its nodes. It does not create or extend the entangled link.</p>'},
