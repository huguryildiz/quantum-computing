...
/* The last pair is noise. Noise carries no colour of its own in this course: it
   is drawn in the hairline tone, so it is a guide by the same argument the grid
   is one, and a label crossing a noise cloud is not a collision. A noise cloud
   drawn as a translucent wash rather than as a hairline is caught further down
   by the `faint` test instead, which reads the alpha off an rgba() fill. */
const GRID = ['#E5E1D8', '#223040', '#DCD7CC', '#C2BCB0', '#27333F', '#3A4754',
              '#B8B2A6', '#44515F'];   /* grid + rule + noise tokens: guides, not content */
  ...
  const sweep = () => page.evaluate(({ GRID, AXIS }) => {
        ...
        svgs.forEach((svg, si) => {
          ...
          /* a light fill is a background plate (box interior, node disc): a label
             sitting on it is by design, not a collision. A decision region is a
             plate in exactly this sense — it is the colour of the symbol it
             decides for at an opacity below a tenth, so it reaches the `faint`
             test below rather than this list. */
          const LIGHTFILL = ['#FFFFFF', '#FAF8F4', '#F2EFE8', '#C2BCB0', '#DCD7CC', '#E5E1D8',
                             '#1A2634', '#0E1621', '#12314E', '#1B4066',
                             '#B8B2A6', '#44515F', 'NONE', ''];
